"""Общее хранилище обязательной подписки (ОП) для нескольких ботов.

Состояние — списки спонсорских ссылок, проверочная ссылка и проверочный
канал — лежит в одном JSON-файле. Все боты на сервере читают этот файл,
а админ-панель в него пишет, поэтому список меняется сразу везде.

Путь задаётся переменной `OP_STATE_FILE` (по умолчанию `op_state.json`
...
# Два независимых списка каналов: на входе в приложение и перед выводом.
KINDS = ("entry", "payout")
...
EMPTY: dict = {
    # списки ссылок по разделам: items:entry / items:payout
    "items:entry": [],
    "items:payout": [],
    "label:entry": "",
    "label:payout": "",
    "updated:entry": "",
    "updated:payout": "",
    # куда попадёт следующий присланный список
    "target": "payout",
    # проверочный канал — один на все разделы, в нём бот администратор
    "check_url": "",
    "check_chat": "",
    "check_title": "",
    "enabled": True,
...
def configure(path: str) -> None:
    """Указать файл состояния (вызывается при старте бота или панели)."""
...
def register_bot(username: str, title: str, bot_id: int,
    ...
    bots_map = dict(read().get("bots") or {})
    entry = dict(bots_map.get(username) or {})
    entry.update({"title": title, "id": bot_id, "seen": now_msk()})
    if can_check is not None:
        entry["can_check"] = bool(can_check)
    bots_map[username] = entry
    update(bots=bots_map)
