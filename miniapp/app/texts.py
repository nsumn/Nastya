...
def sub_proof(status: str, channel: str = "") -> str:
    """Строка для админа: подтверждена ли подписка у автора заявки.

    Нужна потому, что проверка «мягкая»: если Telegram не ответил или
    бот не админ канала, человека пропускают дальше — иначе одна
    сломанная настройка остановила бы всех. Но платить вслепую нельзя,
    поэтому админ видит, чем именно закончилась проверка.
    """
    name = channel or "проверочный канал"
    if status == "on":
        return f"Подписка: ✅ подтверждена ({name})"
    if status == "off":
        return (f"Подписка: ❌ <b>НЕ подписан</b> на «{name}» — "
                "проверять перед выплатой вручную")
    return ("Подписка: ⚠️ <b>проверить не удалось</b> — бот не админ "
            "проверочного канала или Telegram не ответил. Подробности: "
            "/check")
